...
function init() {
  ...
  loader.load('sculptB.obj', function(obj){
    let geo = obj;
    ...
    // ]),
      ...
      fragmentShader: glslify('./src/shaders/main2.frag')
		});
...
//     scene.add( light );
    ...
    let g1 = new THREE.OctahedronGeometry(10,0);
    m1 = new THREE.Mesh(g1, gmaterial);
    scene.add(m1);

    let g2 = new THREE.OctahedronGeometry(10,0);
    ...
    // let g2 = new THREE.OctahedronGeometry(6,2);
    // m2 = new THREE.Mesh(g2, gmaterial);
    // scene.add(m2);
...
function render(){
  if ( video.readyState === video.HAVE_ENOUGH_DATA )
	{
		...
		if ( videoTexture ){
      let data = videoImageContext.getImageData(0, 0,  videoImage.width, videoImage.height);
  ...
  let x;
  if(averageRGB) {
    x = map(((averageRGB.r+ averageRGB.g + averageRGB.b)/3), 0, 255, -1.0, 1.0);
    console.log(x);
  } else {
    x = 0;
  }
  grate.rotation.y += 0.1/(6*x);
  bg.material.opacity = 0.05;
  m1.rotation.z += 0.1/(10.0*Math.max(Math.abs(x), 0.2));
  m2.rotation.z -= 0.1/(10.0*Math.max(Math.abs(x), 0.2));
  // m2.position.y = -2 ;


  camera.updateProjectionMatrix();
  console.log(Math.abs(x)*500);
  camera.fov = Math.max(35, Math.abs(x)*500);
...
function generateTexture() {
	...
	gradient.addColorStop(0, '#00d2ff'); // light blue
	gradient.addColorStop(1, '#3a7bd5'); // dark blue
