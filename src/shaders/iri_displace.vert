...
void main(){
  ...
  float levelX = level * 1000.0;
  // get a turbulent 3d noise using the normal, normal to high freq
  noise = 10.0 *  -.10 * turbulence( .5 * normal  + time + levelX);
  // get a 3d noise using the position, low frequency
  float b = 5.0 * pnoise( 0.05 * position+ vec3( 2.0 * time * levelX ), vec3( 100.0 ) );
  // compose both noises
  float displacement = - 10. * noise + b  * levelX;
